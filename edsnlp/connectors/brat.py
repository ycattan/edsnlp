...
class BratConnector(object):
    ...
    def brat2docs(self, nlp: Language) -> List[Doc]:
        ...
        for doc, doc_annotations in tqdm(
            zip(
                nlp.pipe(texts, batch_size=50, n_process=self.n_jobs),
                annotations
            ),
            ascii=True,
            ncols=100,
    ...
    def doc2brat(self, doc: Doc) -> None:
        ...
        export_to_brat(annotations, self.full_path(f"{filename}.txt"), overwrite_txt=False, overwrite_ann=True)
